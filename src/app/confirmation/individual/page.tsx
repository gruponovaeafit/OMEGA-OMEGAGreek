...
export default function View1() {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(
          result.notification?.message || "Error al guardar disponibilidad.",
        );
        return;
      }
      ...
      toast.success(
        ...
        {
          onClose: () =>
            router.push(result.redirectUrl || "/confirmation/Individual/view2"),
        },
      );
    } catch (error) {
        ...
        <div className="w-full max-w-[320px] mt-10">
          <FormStaticAlert iconName="owl">
            <p className="text-xl text-center">
              El evento se llevará a cabo a partir del sábado{" "}
              <strong>26 de abril a las 11:00 a.m.</strong> hasta el domingo{" "}
              <strong>27 de abril a las 6:00 p.m.</strong>
            </p>
          </FormStaticAlert>
        </div>
        ...
        <div className="mt-5">
          ...
          >
            <span className="text-xs text-gray-200 block text-center max-w-[85%] mx-auto mb-5">
              *Recuerda que en ese lapso de tiempo no podrás salir del lugar
