  ...
  try {
    ...
    try {
      ...
      if (!i) {
        const result = await pool
          .request()
          .input("university", Int, university1).query(`
    ...
    if (!study_area) {
      return res.status(400).json({
        notification: {
          type: "error",
          message: "Faltan datos requeridos area.",
        },
      });
    }

    // Search study area id in study_area_complete
    study_area_id = study_area_complete[0].find(
      (area) => area.area_name === study_area,
    )?.id;
    if (!study_area_id) {
      return res.status(400).json({
        notification: {
          type: "error",
          message: "Área de estudio no encontrada.",
        },
      });
    }
    ...
    try {
      ...
      if (!j) {
        const result = await pool.request().input("area_id", Int, study_area_id)
          .query(`
    ...
    if (career_id === undefined && careers_caeers_complete.length > 0) {
      try {
        ...
        career_id = Array.isArray(careers_caeers_complete[0])
          ? careers_caeers_complete[0].find(
              (carrera: { career_name: string; id: number }) =>
                carrera.career_name === career,
            )?.id
          : undefined;
